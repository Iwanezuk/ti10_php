...
<main class="container">
    ...
    <table class="table table-hover table-condensed tbopacidade">
        ...
        <tbody><!-- corpo da tabela -->
            ...
            <?php // do { ?>
            <tr><!-- linha da tabela -->
                <td class="hidden"><?php echo $rows['id_produto']; ?></td>
                <td>
                    <?php
                        if($rows['destaque_produto']=='Sim'){
                            echo ("<span class='glyphicon glyphicon-heart text-danger' aria-hidden='true'></span>");
                        } else if ($rows['destaque_produto']=='Não'){
                            echo ("<span class='glyphicon glyphicon-ok text-info' aria-hidden='true'></span>");
                        }
                    ?>
                    <?php echo $rows['descri_produto']; ?>
                </td>
                <td><?php echo $rows['resumo_produto']; ?></td>
                <td><?php echo number_format($rows['valor_produto'],2,',','.'); ?></td>
                <!-- 
                ...
                -->
                <td>
                    <img src="../imagens/<?php echo $rows['imagem_produto']; ?>" alt="<?php echo $row['descri_produto']; ?>" width="100px">
                </td>
                <td>
                    <a href="produtos_atualiza.php?id_produto=<?php echo $rows['id_produto']; ?>" class="btn btn-warning btn-block btn-xs">
                        <span class="hidden-xs">ALTERAR <br></span>
                        <span class="glyphicon glyphicon-refresh" aria-hidden="true"></span>
                    </a>
                    <button data-nome="<?php echo $rows['descri_produto'] ?>" data-id="<?php echo $rows['id_produto'] ?>" class="delete btn btn-danger btn-block btn-xs">
            ...
            <?php 
                if ($rows["rotulo_tipo"] != $tipo_atual)
                echo ('<h3 class="breadcrumb alert-danger">'.$rows['rotulo_tipo'].'<br></h3>');
                $tipo_atual = $rows["rotulo_tipo"];
                };
            ?>
